...
describe("Maybe", () => {
    describe("Functor laws", () => {
        ...
        test("Composition: `fmap (f . g) = fmap f . fmap g`", () => {
            ...
            // Additional test with different functions
            {
                ...
                const r = compose(Maybe.fmap(h), compose(Maybe.fmap(f))(Maybe.fmap(g)))(v);
                expect(l).toEqual(r);
            }
        });
    });
    ...
    describe("Applicative", () => {
        ...
        test("Composition: `pure (.) <*> u <*> v <*> w = u <*> (v <*> w)`", () => {
            ...
            const compose = (f: (b: number) => number) => (g: (a: number) => number) => (x: number) => f(g(x));
            const l = Maybe.apply(Maybe.apply(Maybe.apply(Maybe.pure(compose))(u))(v))(w);
            ...
            expect(Maybe.apply(Maybe.apply(Maybe.apply(Maybe.pure(compose))(nothingU))(v))(w)).toEqual(Maybe.apply(nothingU)(Maybe.apply(v)(w)));
            expect(Maybe.apply(Maybe.apply(Maybe.apply(Maybe.pure(compose))(u))(nothingV))(w)).toEqual(Maybe.apply(u)(Maybe.apply(nothingV)(w)));
            expect(Maybe.apply(Maybe.apply(Maybe.apply(Maybe.pure(compose))(u))(v))(nothingW)).toEqual(Maybe.apply(u)(Maybe.apply(v)(nothingW)));
        });
